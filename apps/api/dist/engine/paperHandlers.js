...
function extractEventDate(question) {
    const monthNames = {
...
function isMarketToday(question) {
...
function matchesTimeframe(text, timeframe, startIso, endIso) {
    if (startIso && endIso) {
        const startMs = Date.parse(startIso);
        const endMs = Date.parse(endIso);
        if (Number.isFinite(startMs) && Number.isFinite(endMs)) {
            const diffMin = (endMs - startMs) / 60000;
            if (timeframe === "1h" && diffMin >= 55 && diffMin <= 65)
                return true;
            if (timeframe === "15m" && diffMin >= 13 && diffMin <= 17)
                return true;
            if (timeframe === "5m" && diffMin >= 3 && diffMin <= 7)
                return true;
            if (timeframe === "1m" && diffMin >= 0 && diffMin <= 2)
                return true;
...
const recentCryptoMarketHandler = {
    async execute(node, inputs, ctx) {
        ...
        const filtered = raw
            ...
            if (!matchesTimeframe(hay, timeframe, m.startDate, m.endDate))
                return false;
            return true;
        });
        ...
        if (Array.isArray(market.clobTokenIds) && market.clobTokenIds.length > 0) {
            try {
                const livePrices = await Promise.all(market.clobTokenIds.slice(0, 2).map(async (tokenId) => {
                    const midData = await fetchJson(`${CLOB_HOST}/midpoint?token_id=${tokenId}`);
                    return midData?.mid ?? null;
                }));
                const validPrices = livePrices.filter((p) => p !== null);
        ...
        for (let i = 1; i < values.length; i++) {
            switch (op) {
                ...
                case "*":
                    ...
                    break;
                case "/":
                    result = values[i] !== 0 ? result / values[i] : 0;
                    break;
                default: break;
            }
        }
...
const limitOrderHandler = {
    async execute(node, inputs, ctx) {
        ...
        if (wouldFill) {
            const prevExposure = ctx.state.get("paperExposureUsd") || 0;
            ctx.state.set("paperExposureUsd", prevExposure + sizeUsd);
            ctx.log(node.id, `📝 PAPER LIMIT ${side} ${outcome} | ${shares.toFixed(2)} shares @ limit $${limitPrice.toFixed(3)} — FILLED (market $${currentPrice.toFixed(3)})`);
        }
        else {
        ...
        if (channel === "email") {
            ...
            if (transporter && recipientEmail) {
              ...
              <small style="color:#888;">Strategy: ${ctx.strategyId} | Run: ${ctx.runId}</small>
                    ...
                    ctx.log(node.id, `📧 Email failed: ${err.message}`);
                }
            }
            else {
            ...
            if (token && chatId) {
                try {
                    ...
                    ctx.log(node.id, `📱 Telegram failed: ${err.message}`);
                }
            }
            else {
...
const userActivityHandler = {
    async execute(node, _inputs, ctx) {
        ...
        let activities;
        try {
            activities = (await fetchJson(url));
        }
        catch (err) {
        ...
        const hasRunBefore = ctx.state.get(firstFetchKey);
        if (!hasRunBefore) {
        ...
        // ── Dedup handling ──────────────────────────────────────────────────
        if (ignoreDuplicates) {
            const seenHashes = ctx.state.get(seenHashesKey) || new Set();
        ...
        ctx.log(node.id, `📊 NEW trade → ${tradeSide} ${tradeOutcome} "${tradeTitle.slice(0, 35)}" — $${tradeSize.toFixed(2)} @ ${tradePrice.toFixed(4)} ✅ Signal fired`);
        return {
            market,
            side: tradeSide,
            size: tradeSize,
...
const customApiDataHandler = {
    async execute(node, _inputs, ctx) {
        ...
        try {
            ...
            ctx.log(node.id, `🌐 API error: ${err.message}`);
            return { value: 0, text: "", json: "", signal: null };
        }
    },
};
