...
const priceHistoryHandler: NodeHandler = {
  async execute(node, inputs, ctx) {
    const market = inputs.market as { conditionId?: string } | undefined;
    if (!market?.conditionId) return { prices: [], latest: null };

    const interval = String(node.config.interval || "1h");
    const fidelity = Number(node.config.fidelity || 60);

    const data = await fetchJson(
      `${CLOB_HOST}/prices-history?market=${market.conditionId}&interval=${interval}&fidelity=${fidelity}`,
    );

    const prices = data as Array<{ t: number; p: number }>;
...
const placeOrderHandler: NodeHandler = {
  async execute(node, inputs, ctx) {
    ...
    ctx.log(
      node.id,
      `📝 PAPER ${side} ${outcome} | ${shares.toFixed(2)} shares @ $${fillPrice.toFixed(3)} ($${sizeUsd}) [${orderType}]`,
    );
...
const cancelOrderHandler: NodeHandler = {
  async execute(node, inputs, ctx) {
    const order = inputs.order as { id?: string } | undefined;
