...
export async function ensureApprovals(privateKey: string): Promise<ApprovalResult> {
  ...
  const erc20Spenders: Array<[string, string]> = [
  ...
  for (const [spender, label] of erc20Spenders) {
    try {
      const current: ethers.BigNumber = await usdc.allowance(address, spender);
  ...
  const erc1155Operators: Array<[string, string]> = [
    [CTF_EXCHANGE, "CTF Exchange"],
    [NEG_RISK_CTF_EXCHANGE, "Neg Risk CTF Exchange"],
  ...
  for (const [operator, label] of erc1155Operators) {
    try {
      const isApproved: boolean = await ctf.isApprovedForAll(address, operator);
      if (isApproved) {
