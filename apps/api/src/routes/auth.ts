...
async function checkExpiry(user: DbUser): Promise<DbUser> {
  if (user.tier === "pro" && user.expiresAt && new Date(user.expiresAt) < new Date()) {
    ...
    await usersCol().updateOne(
      { _id: user._id },
      { $set: { tier: "free", subscribedAt: null, expiresAt: null } },
    );
  }
  return user;
}
...
export async function registerAuthRoutes(app: FastifyInstance) {
    ...
    try {
      ...
      if (!user) {
        ...
        if (byEmail) {
          ...
          await usersCol().updateOne(
            { _id: byEmail._id },
            { $set: { googleId: googleUser.sub, avatar: googleUser.picture, name: googleUser.name } },
          );
        ...
        await usersCol().updateOne(
          { _id: user._id },
          { $set: { name: googleUser.name, avatar: googleUser.picture, email: googleUser.email } },
        );
      }
  ...
  app.post("/resend-code", async (req, reply) => {
      ...
      const userId = await resolveSession(token);
      if (!userId) return reply.code(401).send({ error: "Invalid session" });
      ...
      await usersCol().updateOne(
        { _id: user._id },
        { $set: { verificationCode: vCode, verificationCodeExpiresAt: expires } }
      );
  ...
  app.post("/logout", async (req) => {
    const token = (req.headers["x-session-token"] as string) || "";
    if (token) await sessionsCol().deleteOne({ _id: token });
    return { ok: true };
  });
  ...
  app.post("/verify-payment", async (req, reply) => {
    const { userId, txHash, walletAddress } = req.body as {
      userId: string;
      txHash: string;
      walletAddress?: string;
    };
    ...
    app.log.info(
      `User ${user.email} upgraded to Pro (tx: ${txHash}${walletAddress ? `, wallet: ${walletAddress}` : ""})`,
    );
  ...
  app.get("/tier/:userId", async (req, reply) => {
    ...
    const user = await usersCol().findOne({ _id: userId });
    if (!user) return reply.code(404).send({ tier: "free" });
