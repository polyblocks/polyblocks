...
import {
  ...
  marketplaceListingInteractionsCol,
  marketplaceListingStatsCol,
  marketplaceListingViewsCol,
  marketplacePurchasesCol,
  marketplaceVerifiedPerformanceCol,
  paperTradesCol,
  sessionsCol,
  strategiesCol,
  usersCol,
...
async function requireUser(app: FastifyInstance, req: { headers: Record<string, unknown> }) {
  ...
  if (user.tier === "pro" && user.expiresAt && new Date(user.expiresAt) < new Date()) {
    ...
    return { ...user, tier: "free" as const, subscribedAt: null, expiresAt: null };
  }
  return user;
}
...
function safeLowerHex(s: string): string {
...
function toUsdcUnits(amountUsdc: number): ethers.BigNumber {
  if (!Number.isFinite(amountUsdc)) throw new Error("Invalid amount");
...
export function findUsdcTransfer(logs: Array<{ address: string; topics: string[]; data: string }>, toAddress: string, amountUsdc: number) {
  const expectedTo = ethers.utils.getAddress(toAddress);
  const expectedValue = toUsdcUnits(amountUsdc);
  for (const log of logs) {
    if (safeLowerHex(log.address) !== safeLowerHex(POLYGON_USDC_ADDRESS)) continue;
...
async function getListingStatsMap(listingIds: string[]) {
  const stats = await marketplaceListingStatsCol().find({ _id: { $in: listingIds } }).toArray();
  const m = new Map<string, (typeof stats)[number]>();
  for (const s of stats) m.set(s._id, s);
  return m;
}

async function isWalletLinked(userId: string, walletAddress: string) {
...
async function ensureMarketplaceSeeded() {
  ...
  for (let i = 0; i < templatesToSeed.length; i += 1) {
    ...
    await marketplaceVerifiedPerformanceCol().updateOne(
      { listingId },
      {
        $set: {
          computedAt: nowIso,
...
function computeTradeMetrics(trades: Array<{ side: "BUY" | "SELL"; price: number; size: number; tokenId: string; executedAt: string }>) {
...
export async function registerMarketplaceRoutes(app: FastifyInstance) {
  app.get("/listings", async (req) => {
    ...
    const { limit, offset, search, sort } = req.query as { limit?: string; offset?: string; search?: string; sort?: string };
    const take = Math.max(1, Math.min(50, Number(limit) || 20));
    const skip = Math.max(0, Number(offset) || 0);

    const filter: Record<string, unknown> = { status: "active", visibility: "public" };
  ...
  app.get<{ Params: { listingId: string } }>("/listings/:listingId", async (req, reply) => {
  ...
  app.post<{ Params: { listingId: string } }>("/listings/:listingId/view", async (req, reply) => {
    const { listingId } = req.params;
    const listing = await marketplaceListingsCol().findOne({ _id: listingId });
    if (!listing) return reply.code(404).send({ error: "Listing not found" });
    ...
    await marketplaceListingStatsCol().updateOne(
      { _id: listingId },
      { $inc: inc, $set: { updatedAt: nowIso }, $setOnInsert: { _id: listingId, views: 0, uniqueViews: 0, likes: 0, upVotes: 0, downVotes: 0, purchases: 0 } },
      { upsert: true },
    );
  ...
  app.post<{ Params: { listingId: string } }>("/listings/:listingId/like", async (req, reply) => {
    const user = await requireUser(app, req);
    ...
    await marketplaceListingInteractionsCol().updateOne(
      { listingId, userId: user._id },
      {
        $set: { liked: nextLiked, vote: (existing?.vote ?? 0) as -1 | 0 | 1, updatedAt: nowIso },
    ...
    await marketplaceListingStatsCol().updateOne(
      { _id: listingId },
      { $inc: { likes: nextLiked ? 1 : -1 }, $set: { updatedAt: nowIso }, $setOnInsert: { _id: listingId, views: 0, uniqueViews: 0, likes: 0, upVotes: 0, downVotes: 0, purchases: 0 } },
      { upsert: true },
    );
  ...
  app.post<{ Params: { listingId: string } }>("/listings/:listingId/vote", async (req, reply) => {
    const user = await requireUser(app, req);
    if (!user) return reply.code(401).send({ error: "Not authenticated" });
    const { listingId } = req.params;
    const listing = await marketplaceListingsCol().findOne({ _id: listingId });
    ...
    const nowIso = new Date().toISOString();
    const existing = await marketplaceListingInteractionsCol().findOne({ listingId, userId: user._id });
    const prevVote = (existing?.vote ?? 0) as -1 | 0 | 1;
    const nextVote = vote as -1 | 0 | 1;

    const inc: Record<string, number> = {};
    if (prevVote === 1) inc.upVotes = (inc.upVotes || 0) - 1;
    if (prevVote === -1) inc.downVotes = (inc.downVotes || 0) - 1;
    if (nextVote === 1) inc.upVotes = (inc.upVotes || 0) + 1;
    if (nextVote === -1) inc.downVotes = (inc.downVotes || 0) + 1;
  ...
  app.post("/wallet/verify", async (req, reply) => {
    const user = await requireUser(app, req);
    if (!user) return reply.code(401).send({ error: "Not authenticated" });
    const body = (req.body || {}) as { walletAddress?: string; signature?: string; nonce?: string };
    const walletAddress = typeof body.walletAddress === "string" ? body.walletAddress.trim() : "";
    const signature = typeof body.signature === "string" ? body.signature.trim() : "";
    const nonce = typeof body.nonce === "string" ? body.nonce.trim() : "";
    if (!walletAddress || !signature || !nonce) return reply.code(400).send({ error: "walletAddress, signature, nonce required" });
    ...
    if (!challenge) return reply.code(400).send({ error: "Challenge not found" });
    ...
    if (safeLowerHex(recovered) !== safeLowerHex(checksummed)) return reply.code(400).send({ error: "Signature does not match wallet" });

    const nowIso = new Date().toISOString();
    await walletLinksCol().updateOne(
      { walletAddress: checksummed },
      { $set: { userId: user._id, walletAddress: checksummed, verifiedAt: nowIso }, $setOnInsert: { _id: nanoid() } },
      { upsert: true },
    );
    await walletChallengesCol().deleteOne({ _id: challenge._id });
  ...
  app.post("/listings", async (req, reply) => {
    ...
    if (!sourceStrategyId) return reply.code(400).send({ error: "sourceStrategyId required" });
    ...
    if (strategy.userId !== user._id) return reply.code(403).send({ error: "Not strategy owner" });
    ...
    if (!Number.isFinite(priceUsdc) || priceUsdc <= 0) return reply.code(400).send({ error: "priceUsdc must be > 0" });

    const walletAddress = typeof body.creatorWalletAddress === "string" ? body.creatorWalletAddress.trim() : "";
    if (!walletAddress) return reply.code(400).send({ error: "creatorWalletAddress required" });
    ...
    if (!walletLink) return reply.code(400).send({ error: "Wallet not verified for this user" });
  ...
  app.post<{ Params: { listingId: string } }>("/listings/:listingId/purchase", async (req, reply) => {
    const user = await requireUser(app, req);
    if (!user) return reply.code(401).send({ error: "Not authenticated" });
    const { listingId } = req.params;
    const listing = await marketplaceListingsCol().findOne({ _id: listingId });
    if (!listing) return reply.code(404).send({ error: "Listing not found" });
    if (listing.ownerUserId === user._id) return reply.code(400).send({ error: "Cannot purchase your own listing" });
    const body = (req.body || {}) as { payerWalletAddress?: string };
    const payerWalletRaw = typeof body.payerWalletAddress === "string" ? body.payerWalletAddress.trim() : "";
    if (!payerWalletRaw) return reply.code(400).send({ error: "payerWalletAddress required" });
    ...
    if (!buyerLinked) return reply.code(400).send({ error: "Wallet not verified for this user" });

    const sellerLinked = await walletLinksCol().findOne({ walletAddress: listing.creatorWalletAddress });
    if (!sellerLinked) return reply.code(400).send({ error: "Seller wallet not verified" });
  ...
  app.post<{ Params: { purchaseId: string } }>("/purchases/:purchaseId/verify", async (req, reply) => {
    const user = await requireUser(app, req);
    if (!user) return reply.code(401).send({ error: "Not authenticated" });
    ...
    if (purchase.buyerUserId !== user._id) return reply.code(403).send({ error: "Not purchase owner" });
    ...
    if (!tx || !receipt) return reply.code(400).send({ error: "Transaction not found" });
    if (receipt.status !== 1) return reply.code(400).send({ error: "Transaction failed" });

    const match = findUsdcTransfer(receipt.logs as Array<{ address: string; topics: string[]; data: string }>, COMMISSION_WALLET_ADDRESS, purchase.amountUsdc);
    if (!match) return reply.code(400).send({ error: "Expected USDC transfer not found" });
    ...
    const nowIso = new Date().toISOString();
    try {
      await marketplacePurchasesCol().updateOne(
        { _id: purchaseId },
        { $set: { status: "verified", txHash, payerAddress, verifiedAt: nowIso } },
      );
    } catch {
    ...
    await marketplaceListingStatsCol().updateOne(
      ...
      { $inc: { purchases: 1 }, $set: { updatedAt: nowIso }, $setOnInsert: { _id: purchase.listingId, views: 0, uniqueViews: 0, likes: 0, upVotes: 0, downVotes: 0, purchases: 0 } },
      { upsert: true },
    );
    ...
    await marketplacePurchasesCol().updateOne(
      { _id: purchaseId },
      { $set: { clonedStrategyId: strategyId } },
    );
  ...
  app.post<{ Params: { listingId: string } }>("/listings/:listingId/clone", async (req, reply) => {
    const user = await requireUser(app, req);
    if (!user) return reply.code(401).send({ error: "Not authenticated" });
    const { listingId } = req.params;
    const listing = await marketplaceListingsCol().findOne({ _id: listingId });
    ...
    if (!isOwner) {
      ...
      if (!purchase) return reply.code(403).send({ error: "Purchase required" });
    }
  ...
  app.post<{ Params: { listingId: string } }>("/listings/:listingId/performance/compute", async (req, reply) => {
    const user = await requireUser(app, req);
    if (!user) return reply.code(401).send({ error: "Not authenticated" });
    const { listingId } = req.params;
    const listing = await marketplaceListingsCol().findOne({ _id: listingId });
    if (!listing) return reply.code(404).send({ error: "Listing not found" });
    if (listing.ownerUserId !== user._id) return reply.code(403).send({ error: "Not listing owner" });
  ...
  app.get<{ Params: { listingId: string } }>("/listings/:listingId/performance", async (req, reply) => {
    const { listingId } = req.params;
    const listing = await marketplaceListingsCol().findOne({ _id: listingId });
    if (!listing) return reply.code(404).send({ error: "Listing not found" });
    const perf = await marketplaceVerifiedPerformanceCol().findOne({ listingId });
    if (!perf) return reply.code(404).send({ error: "No verified performance yet" });
    return {
      listingId,
  ...
  app.get("/verified", async () => {
    ...
    for (const l of listings) listingMap.set(l._id, l);
    return {
      verified: perfs
        .map((p) => {
