...
function makeUser(id: string, tier: "free" | "pro") {
  return {
    _id: id,
...
describe("marketplace", () => {
  it("verifies wallet via challenge + signature", async () => {
    ...
    const challenge = challengeRes.json() as { nonce: string; message: string };
    ...
    const v = verifyRes.json() as { verified: boolean; walletAddress: string };
  ...
  it("tracks views and unique views", async () => {
    ...
    const { id } = pub.json() as { id: string };
  ...
  it("requires verified purchase to clone for non-owners", async () => {
    ...
    const { id: listingId } = pub.json() as { id: string };
    ...
    const data = ok.json() as { strategyId: string };
