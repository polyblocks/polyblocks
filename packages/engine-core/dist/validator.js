 ...
 */
import { ValidationSeverity, BlockType, NodeCategory, BLOCK_REGISTRY, } from "@polyblocks/types";
...
export function validateStrategy(graph) {
    ...
    if (topo.hasCycle) {
        issues.push({
            severity: ValidationSeverity.Error,
            message: "Strategy contains a cycle. Connections must form a directed acyclic graph (DAG).",
        });
    }
    // ── 3. Must have at least one trigger ─────────────────────────────────────
    const hasTrigger = graph.nodes.some((n) => BLOCK_REGISTRY[n.type]?.category === NodeCategory.Trigger);
    if (!hasTrigger) {
        issues.push({
            severity: ValidationSeverity.Error,
            message: "Strategy needs at least one Trigger block (Interval, Price Cross, or Manual).",
        });
    }
    // ── 4. Must have at least one action ──────────────────────────────────────
    const hasAction = graph.nodes.some((n) => BLOCK_REGISTRY[n.type]?.category === NodeCategory.Action);
    if (!hasAction) {
        issues.push({
            severity: ValidationSeverity.Warning,
            message: "Strategy has no Action blocks. It will run but won't do anything.",
        });
    }
    // ── 5. Check for disconnected nodes ───────────────────────────────────────
    const adj = buildAdjacency(graph.nodes, graph.edges);
    for (const node of graph.nodes) {
        if (node.type === BlockType.Note)
            continue; // Notes are always disconnected
        ...
        const isSource = def.category === NodeCategory.Trigger ||
        ...
        const isLeaf = def.category === NodeCategory.Action ||
    ...
    const hasInput = (nodeId, portId) => graph.edges.some((e) => e.target === nodeId && e.targetHandle === portId);
    const hasOutput = (nodeId, portId) => graph.edges.some((e) => e.source === nodeId && e.sourceHandle === portId);
    for (const node of graph.nodes) {
        switch (node.type) {
...
function isPortCompatible(source, target) {
    if (source === target)
        return true;
    if (source === "any" || target === "any")
        return true;
    // Signal can loosely connect to boolean (truthy)
    if (source === "signal" && target === "boolean")
        return true;
    return false;
}
